...
knitr::opts_chunk$set(message = FALSE, warning = FALSE, paged.print=FALSE, echo=FALSE, dev=c('svg', 'png'))
...
filt_alpha %>%
  ...
  geom_boxplot(aes(fill=timepoint), width=0.8, alpha=.8) +
  xlab('Timepoint') +
  ylab('Chao1 Index') +
...
```{r Plot ordination, fig.cap="Fig 3B: Beta diversity differences pre and post challange T2 vs T3", fig.dim = c(6,5)}
...
```{r fig.cap="Fig 3C: Differential abundance analysis highlighted Bacteroidales S24/7 and Lacnospiraceae as most differential abunant families.", fig.dim=c(6,5)}
...
ggplot(data=lacno_bact_relabs, aes(x=timepoint, y=Abundance)) +
  stat_boxplot(aes(fill=timepoint), alpha=.8) +
  facet_wrap(~taxlab) +
...
```{r fig.cap="Fig 3D: Ordination centroids beta diversity all timepoints", fig.dim = c(6,5)}
not12 <- gg_topi %>%
...
```{r fig3E, fig.cap="Fig 3E: Boxplot of abundance of significant genera from DESeq2 differential analysis. Genera with median below 1% are filtered", fig.height=6}
genus_ab_t %>%
