...
ggplot(aes(Axis.1, Axis.2)) +
  ...
  scale_color_brewer(name = "Timepoint", palette="Dark2") + 
  theme_minimal() +
  annotation_custom(grob = grid::textGrob(bquote(atop("p-value"==.(permanova$aov.tab$`Pr(>F)`[1]),R^2==.(signif(permanova$aov.tab$R2[1], 2)))), gp=grid::gpar(col="black", fontsize=10)),  xmin = 0, xmax = 0, ymin = .1, ymax = .1)
```
...
```
## Panel E: Boxplot of abundance of significant genera from DESeq2 differential analysis
```{r}
...
ggplot() +
  geom_boxplot(aes(reorder(interaction(Family, Genus), Abundance, FUN = median), Abundance, fill=timepoint)) + 
