...
```{r fig3a, fig.cap='Fig 3A: Alpha diversity controls T1 vs T2 and pre and post challenge T2 vs T3', fig.height=3, fig.width=3}
...
```{r fig3b, fig.cap="Fig 3B: Beta diversity differences pre and post challange T2 vs T3", fig.dim = c(6,5)}
...
```{r fig3c, fig.cap="Fig 3C: Differential abundance analysis highlighted Bacteroidales S24/7 and Lacnospiraceae as most differential abunant families.", fig.dim=c(6,5)}
...
```{r fig3d, fig.cap="Fig 3D: Ordination centroids beta diversity all timepoints", fig.dim = c(6,5)}
not12 <- gg_topi %>%
...
```{r fig3g, include=FALSE, eval=FALSE, fig.height=3,fig.cap="Fig 3 G: Scatterplot of tumor size and difference of beta diversity "}
distancesbc %>% 
  dplyr::filter(topo1==topo2) %>%
  dplyr::filter(timepoint1==3) %>%
  dplyr::filter(timepoint2>4) %>%
  ...
  theme_cm() +
  geom_text(data = ds_corr_braycf, aes(x=x,y=y, label=label))
```


```{r fig3i, fig.cap="Fig 3I: Scatterplot of tumor size and difference of beta diversity ", fig.height=3}
