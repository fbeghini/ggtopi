...
```{r supplfig3a, fig.cap='Fig 3A: Alpha diversity controls T1 vs T2 and pre and post challenge T2 vs T3'}
filt_alpha %>%
...
```{r Plot ordination, fig.cap="Fig 3B: Beta diversity differences pre and post challange T2 vs T3"}
...
```{r fig.cap="Fig 3C: Differential abundance analysis highlighted Bacteroidales S24/7 and Lacnospiraceae as most differential abunant families."}
  lacno_bact_relabs %>%
  ggplot() +
...
```{r fig.cap="Fig 3D: Ordination centroids beta diversity all timepoints"}
not12 <- gg_topi %>%
...
```{r fig3E, fig.cap="Fig 3E: Boxplot of abundance of significant genera from DESeq2 differential analysis. Genera with median below 1% are filtered"}
genus_ab_t %>%
...
```{r supplfig3E, fig.cap="Fig 3E: Boxplot of abundance of significant genera from DESeq2 differential analysis"}
genus_ab_t %>%
  bind_cols(data.frame(tax_table(ggtopi_subs.genus.relab)[genus_ab_t$OTU,c('Phylum','Family','Genus')])) %>%
  filter(Phylum != "Unassigned") %>%
  mutate(taxlab = ifelse(stringr::str_detect(Genus,'uncultured'), paste('Uncultured',Family,sep = ' '), paste(Genus))) %>%
...
ggplot() +
  ...
  ggsignif::geom_signif(comparisons = list(c('T2','T3')), y_position = 0.5, tip_length = 0, test = 't.test') +
...
```{r fig3G, corr tumor braycurtis, fig.cap="Fig 3 G: Scatterplot of tumor size and difference of beta diversity "}
distancesbc %>% 
  dplyr::filter(topo1==topo2) %>%
  dplyr::filter(timepoint1==3) %>%
  dplyr::filter(timepoint2>4) %>%
...
```{r corr tumor braycurtis, fig.cap="Fig 3I: Scatterplot of tumor size and difference of beta diversity "}
