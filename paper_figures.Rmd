...
```{r Plot boxplot, fig.cap='Fig 3A: Alpha diversity controls T1 vs T2 and pre and post challenge T2 vs T3'}
filt_alpha %>%
...
```{r fig.cap="Fig 3E: Boxplot of abundance of significant genera from DESeq2 differential analysis. Genera with median below 1% are filtered"}
genus_ab_t %>%
...
```{r fig.cap="Fig 3 F: Abundance of Bacteroidales 24/7 and Lachnospiraceae NK4A136 through timepoints and treatment"}
...
```{r corr tumor braycurtis, fig.cap="Fig 3 G: Scatterplot of tumor size and difference of beta diversity "}
distancesbc %>% 
  dplyr::filter(topo1==topo2) %>%
  dplyr::filter(timepoint1==3) %>%
  dplyr::filter(timepoint2>4) %>%
