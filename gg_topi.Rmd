...
dds <- DESeq2::DESeq(phyloseq_to_deseq2(ggtopi_filt, design = ~ cond_vac), parallel = TRUE, quiet = TRUE, fitType = 'parametric')
...
## Differential analysis Pentatome T7 vs not vaccinated T7
```{r}
  res12 <- DESeq2::results(dds, contrast = c("cond_vac","T7_YES","T7_NO"), independentFiltering = TRUE)
  res12.filtered <- res12[!is.na(res12$padj),]
  res12.filtered <- res12.filtered[res12.filtered$padj < threshold,]
...
```{r}
  res12 <- DESeq2::results(dds, contrast = c("cond_vac","T7_YES","T3_NO"))
  res12.filtered <- res12[!is.na(res12$padj),]
  res12.filtered <- res12.filtered[res12.filtered$padj < threshold,]
...
res12.filtered %>% 
  as.data.frame %>% 
  cbind(as.matrix(tax_table(gg_topi)[rownames(res12.filtered),])) %>%
  filter(is.na(Phylum)) %>%  knitr::kable()
```
